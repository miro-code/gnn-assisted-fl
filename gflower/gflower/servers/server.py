...
class NewHistory(History):
    ...
    def __init__(self) -> None:
        ...
        self.eval_metrics_distributed: Dict[str, List[Tuple[int, Scalar]]] = {}
        self.metrics_distributed: Dict[str, List[Tuple[int, Scalar]]] = {
            "Placeholder, see fit and eval metrics distributed results in hist object": [
                1,
                1,
            ]
        }
    ...
    def add_eval_metrics_distributed(
        self, server_round: int, metrics: Dict[str, Scalar]
    ) -> None:
        """Add metrics entries (from distributed evaluation)."""
        for key in metrics:
            # if not (isinstance(metrics[key], float) or isinstance(metrics[key], int)):
            #     continue  # ignore non-numeric key/value pairs
            if key not in self.eval_metrics_distributed:
                self.eval_metrics_distributed[key] = []
            self.eval_metrics_distributed[key].append((server_round, metrics[key]))
    ...
    def __repr__(self) -> str:
        ...
        if self.eval_metrics_distributed:
        ...
        for current_round in range(1, num_rounds + 1):
            ...
            if res_fed:
                ...
                if loss_fed:
                    ...
                    history.add_eval_metrics_distributed(
